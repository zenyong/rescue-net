  ...
  try {
    ...
    const geocodingResponse = await axios.get(
      `https://api.mapbox.com/geocoding/v5/mapbox.places/${encodeURIComponent(
        address
      )}.json?access_token=${"pk.eyJ1IjoibWVudXRvayIsImEiOiJjbGMzeG1kNHgwbm9lNDFuemkzaXduazgxIn0.alLoPOMYiPcZtHmA9EaLZQ"}`
    );
...
export const loginAgency = async (req, res) => {
  try {
    ...
    //token
    const token = await jwt.sign({ _id: user._id }, "rANdomStr1nGwiTHenouGHenc0d1ng$$", {
      expiresIn: "7d",
    });
    res.status(200).send({
      success: true,
